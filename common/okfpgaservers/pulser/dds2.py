...
class DDS(LabradServer):
    ...
    @setting(45, 'Add DDS Pulses',  values = ['*(sv[s]v[s]v[MHz]v[dBm]v[deg]v[MHz]v[dbm])'])
    def addDDSPulses(self, c, values):
        '''
        input in the form of a list [(name, start, duration, frequency, amplitude, phase, freq_ramp_rate,amp_ramp_rate)]
        '''
        ...
        for value in values:
            if len(value) == 5:
                ...
                name,start,dur,freq,ampl,phase,freq_ramp_rate,amp_ramp_rate = value
            try:
            ...
            freq = freq['MHz']
            ampl = ampl['dBm']
            freq_ramp_rate = freq_ramp_rate['MHz']
            ...
            num = self.settings_to_num(channel, freq, ampl, phase, freq_ramp_rate,amp_ramp_rate)
    ...
    def settings_to_num(self, channel, freq, ampl, phase = 0.0, freq_ramp_rate = 0.0, amp_ramp_rate = 0.0):
        if not channel.phase_coherent_model:
            num = self._valToInt(channel, freq, ampl)
        else:
            num = self._valToInt_coherent(channel, freq, ampl, phase, freq_ramp_rate,amp_ramp_rate)
        return num
    ...
    def _valToInt_coherent(self, channel, freq, ampl, phase = 0, freq_ramp_rate=0, amp_ramp_rate=0):
        '''
        takes the frequency and amplitude values for the specific channel and returns integer representation of the dds setting
        freq is in MHz
        power is in dbm
...
#       profile = 7
        ...
        ### add ramp rate 
        minim, maxim = channel.board_freq_ramp_range
        resolution = (maxim - minim) / float(2**16 - 1)
        if freq_ramp_rate < minim: ### if the ramp rate is smaller than the minim, thenn treat it as no rampp
            seq = 0
        elif freq_ramp_rate > maxim:
            seq = 2**16-1
        else:
