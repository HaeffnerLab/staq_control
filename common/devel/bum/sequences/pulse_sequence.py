...
#         print carrier_translation[carrier]
    ...
    @classmethod
    def parity_sin_fit(cls, phase, excitation, return_all_params = False, init_guess = None):
        if init_guess == "stop":
            return None

        model = lambda x, a, x0, c: a*np.sin((2.*np.pi*x/180.)+x0) + c
        force_guess = False
    ...
    @classmethod
    def get_checked_parameters(cls): # EP
        li = [] 
        cls.loop_get_checked(li)
        return li 

    @classmethod
    def loop_get_checked(cls, li): # EP
        scan = cls.checked_params
        for item in scan:
            li.append((item, cls.__name__))
            ...
            if type(cls.sequence) == list:
                for subcls in cls.sequence:
                    if type(subcls) == tuple:
                        subcls = subcls[0]
                    subcls.loop_get_checked(li)
            elif type(cls.sequence) == type:
                cls.sequence.loop_get_checked(li)
        except:
            print "Loop_get_checked: error with ", cls.__name__
            return
