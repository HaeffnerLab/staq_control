...
class ScriptScanner(ParameterVault, Signals, ParameterLogger, LabradServer):
    ...
    @setting(10, 'New Sequence', sequence_name = 's', settings = '*(s(svvvs))',priority = 's', returns = 'w')
    def new_sequence(self, c, sequence_name, settings,priority='Normal'):
        '''
        Launch a new sequence. Returns ID of the queued scan.
        settings = [(sequence_name, [scan param, minim, maxim, steps, unit])]
        '''
        print "LAUNCHING A NEW SEQUENCE", sequence_name, settings
        ...
        scan_id = self.scheduler.add_scan_to_queue(wrapper,priority)
...
#                print 'waiting to unfinish', ident
    ...
    @setting(47,"Get Current Sequence Settings", returns = '*(s(svvvs))') # EP
    def get_current_sequence_settings(self, c):
        '''
        Returns the settings for the current sequence
        '''
        current_sequence_wrap = self.scheduler.get_current_running()
        return current_sequence_wrap.get_settings() 
    ...
    @setting(46, "Get Sequence Checked", sequence = 's', returns = '*(ss)') # EP
    def get_sequence_checked(self, c, sequence):
        try:
            #print sequence
            #print self.sequences[sequence].get_checked_parameters()
            return self.sequences[sequence].get_checked_parameters()
        except KeyError:
            raise Exception('Sequence not found')
    ...
    @setting(44, 'Emit scannable modified', returns = '') # EP 
    def emit_scannable_modified(self, c, arg):
        self.on_scannable_modified(arg)
...
#*********************************************************************************
    # @setting(49, 'Test', value = '?', returns = '') # EP 
    # def test(self, c, value):
        ...
        # self.on_parameter_changed((key[0], key[1]), notified)

    # @setting(50, 'Test2', value = '?', returns = '') # EP 
    # def test2(self, c, value):
    #     self.on_scannable_modified(('Calibrations_CalibAllLines', 'CalibLine2', 'Excitation729.frequency729', (-value, value, 10.0)))
#*********************************************************************************
