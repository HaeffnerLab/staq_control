...
class scheduler(object):
    ...
    def get_current_running(self): # EP
    ...
    def launch_scripts(self, result = None):
        ...
        try:
            ...
            if not [s for s in self.running.itervalues() if s.status.status != 'Paused']:
                #no scripts are running and unpaused, or current one has no conflicts
                should_launch = True
    ...
    def do_launch(self, ident, scan, priority, pause_running_not_a_function):
        ...
        print "do_launch: {}".format(pause_running_not_a_function), "for scan ident:",ident
        d = Deferred()  
        scan.update_params(self.parent.all_parameters())
        status = script_semaphore(ident, self.signals)
        self.current_running = scan # EP 
