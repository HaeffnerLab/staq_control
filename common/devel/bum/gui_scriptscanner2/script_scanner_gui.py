...
class script_scanner_gui(QtGui.QWidget):
    ...
    def populateExperiments(self):
        ...
        for experiment in available:
            ...
            checked_params = yield sc.get_sequence_checked(experiment)
            self.scan_widget.buildSequenceWidget(experiment, scannable_params, checked_params)
    ...
    def setupListenersScriptScanner(self):
        ...
        yield sc.signal_on_scannable_modified(self.SIGNALID + 11, context = self.context) # EP
        ...
        if not self.subscribedScriptScanner:
            ...
            yield sc.addListener(listener = self.on_scannable_modified, source = None, ID = self.SIGNALID + 11, context = self.context) # EP          
    ...
    def on_scannable_modified(self, signal, info): # EP
        experiment, sequence_name, param, values = info
        self.scan_widget.set_parameter_values(experiment, sequence_name, param, values)

    def on_pv_parameter_change(self, signal, info): # EP
        collection, name, full_info = info
    ...
    # def on_pv_parameter_change(self, signal, info): # OLD VERSION
    #     print 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa on_pv_parameter_change' 
