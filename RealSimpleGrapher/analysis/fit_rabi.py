...
class Rabi(Model):
    ...
    def __init__(self):
        self.parameters = {
            ...
            'eta':ParameterInfo('eta', 2, lambda x, y: 0.07, vary=False),
    ...
    def model(self, x, p):
        ...
        omega_coupling = rc.compute_rabi_coupling(eta, sideband_order, nmax)
        ones = np.ones_like(x)
        ...
        effective_omega = np.sqrt((omega_rabi*omega_coupling)**2+delta**2)
                
        result = np.outer(p_n * (omega_rabi*omega_coupling)**2/effective_omega**2, ones) * (np.sin( np.outer( effective_omega/2, x ))**2)
